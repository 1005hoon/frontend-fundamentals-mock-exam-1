...
export function getCalculationResultFromSavingsProduct(
  ...
  const expectedTotalAmount = monthlyAmount * termMonths * (1 + annualRate * COMMON_INTEREST_RATE);
  const differenceFromTarget = targetAmount - expectedTotalAmount;
  const recommendedMonthlyAmount =
    Math.round(targetAmount / (termMonths * (1 + annualRate * COMMON_INTEREST_RATE)) / 1000) * 1000;
